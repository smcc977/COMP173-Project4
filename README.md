...
| `out.txt` | Reference/expected output provided with the assignment, used to verify correctness. |
| `output.txt` | Output file generated by the program at runtime. Contains the same trace as stdout. |
| `Makefile` | Build file (targets Linux kernel module build; use the manual `gcc` command below on other systems). |
| `mmu-copy.c` | Working copy used during development. Not part of the final submission. |
...
The program writes a full address trace to both stdout and `output.txt`, then prints aggregate statistics:
